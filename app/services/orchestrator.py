...
from app.prompts.orchestrator import get_orchestrator_prompt
from app.prompts.extraction import (
    get_company_name_prompt,
    get_entity_extraction_prompt,
)
from app.schemas.meeting import EntityExtraction, TranscriptMetadata
from app.services.backboard import BackboardService, get_backboard_service
from app.tools.definitions import get_orchestrator_tools
...
class OrchestratorService:
    ...
    async def ensure_orchestrator_exists(self) -> str:
        """Ensure the orchestrator assistant exists, creating if needed.
        ...
        # Look for existing orchestrator
        assistant = await self._backboard.find_assistant_by_name(
            self._settings.orchestrator_assistant_name
        )

        if assistant:
            self._assistant_id = assistant.assistant_id
            desired_prompt = get_orchestrator_prompt()
            existing_prompt = getattr(assistant, "description", None)
            if existing_prompt != desired_prompt:
                await self._backboard.update_assistant(
                    assistant_id=self._assistant_id,
                    system_prompt=desired_prompt,
                )
            # Load customer registry from memory
            await self._load_customer_registry()
            return self._assistant_id

        # Create new orchestrator (tools disabled for now - can cause API issues)
        assistant = await self._backboard.create_assistant(
            name=self._settings.orchestrator_assistant_name,
            system_prompt=get_orchestrator_prompt(),
            # tools=get_orchestrator_tools(),  # Disabled - can cause Backboard API issues
        )

        self._assistant_id = assistant.assistant_id

        # Initialize customer registry in memory
        await self._save_customer_registry()
        ...
        try:
            ...
            for memory in memories.memories:
                try:
                    ...
                    if data.get("type") == "customer_registry":
                        for customer in data.get("customers", []):
                            normalized = self._normalize_company_name(
                                customer["company_name"]
                            )
                            self._customer_registry[normalized] = customer["assistant_id"]
                        break
                except (json.JSONDecodeError, KeyError):
                    continue
        except Exception:
            # Start with empty registry if can't load
            self._customer_registry = {}

    async def _save_customer_registry(self) -> None:
        """Save the customer registry to orchestrator memory."""
        if not self._assistant_id:
            return

        registry_data = {
            "type": "customer_registry",
        ...
        # Check if registry memory exists
        try:
            memories = await self._backboard.get_memories(self._assistant_id)
            for memory in memories.memories:
                try:
    ...
    async def extract_entities(
        ...
        try:
            ...
            if json_match:
                ...
                from app.schemas.meeting import StakeholderMention
                contacts = []
    ...
    async def route_transcript(
        ...
        entities = await self.extract_entities(transcript, metadata)
